...
def weight_matrix_with_removals_and_subsetting(matrix: dict):
    ...
    for pokemon, v in victories:
        if any(v.issubset(w[1]) and v != w[1] for w in victories):
            to_remove.append((matrix[pokemon]['row'], pokemon))

    if to_remove:
        to_remove.sort()
        return matrix, [x[1] for x in to_remove]
...
def ordered_top_pokemon(cup: str, subsetting=False):
    query = Query()
    db = TinyDB(f"{path}/data/databases/{cup}.json")
    table = db.table('meta_with_subsetting' if subsetting else 'meta')
...
def all_pokemon_movesets(cup: str, filter=False):
    query = Query()
    db = TinyDB(f"{path}/data/databases/{cup}.json")
    table = db.table('meta')
    ...
    if filter:
        data = [x for x in data if x['absolute_rank'] < 99.9]
    return data


def ordered_movesets_for_pokemon(cup: str, pokemon: str, subsetting=False):
    query = Query()
    db = TinyDB(f"{path}/data/databases/{cup}.json")
    table = db.table('meta_with_subsetting' if subsetting else 'meta')
    ...
    if filter:
        data = [x for x in data if x[0] < 99.9]
    return data
